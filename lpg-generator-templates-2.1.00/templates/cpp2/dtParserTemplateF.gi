...
%Headers
     ...
     struct $action_type :public $super_class ,public RuleAction$additional_interfaces
    {
         ...
         static ParseTable* prsTable = new $prs_type();
         ...
         void reset(ILexStream* lexStream)
        {
            ...
            catch(UnimplementedTerminalsException& e)
            {
                if (unimplementedSymbolsWarning) {
                    ...
                    {
                        auto id = (Integer) unimplemented_symbols[i];
                        std::cout <<("    " << $sym_type::orderedTerminalSymbols[id]);               
                    }
                    std::cout << std::endl;
                }
            }
            ...
            {
                throw  Error( UndefinedEofSymbolException
         ...
         $action_type(ILexStream* lexStream = nullptr)
        {
            ...
            {
                throw  Error(new BadParseSymFileException("Bad Parser Symbol File -- $sym_type.cpp. Regenerate $prs_type.cpp"));
            }
			if(lexStream){
				reset(lexStream);
			}
         ...
         std::vector< std::wtring> orderedTerminalSymbols() { return $sym_type::orderedTerminalSymbols; }
         std::wtring getTokenKindName(int kind) { return $sym_type::orderedTerminalSymbols[kind]; }            
